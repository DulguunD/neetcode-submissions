class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for right, char in enumerate(s2):
            found[char] += 1
            # print(f"left: {left}, right: {right}, found: {found}, matches: {matches}")
            ...
            # if matches == required: 
            #     print(f"\tmatches == required: {matches}, total dict values: {total}")
            #     # print(f"\tFOUND: {found}")

            # if total == required: 
            #     print(f"totaldict == required: {total}, matches value: {matches}")
            #     # print(f"FOUND: {found}")

            if matches == required and right-left+1==required:
            # if matches == required and sum(found.values()) == required:
            # if matches == required:
            # if sum(found.values()) == required:
            # if matches == len(need):
                # print(f"matches: {matches}, sum: { sum(found.values())}, required: {required}")
                # print(f"left: {left}, right: {right}, found: {found}")
                # print(f"SOLUTION FOUND: {found}, matches: {matches}, length: {right-left+1}")
                return True
